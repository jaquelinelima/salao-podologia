<template>
  ...
  <div class="m-10 mx-auto p-16 sm:p-24 lg:p-48 bg-gray-200">
    ...
    <div class="mx-auto mt-16 max-w-6xl flex-col grid grid-cols-3 gap-8 lg:mx-0 lg:mt-10 lg:max-w-none lg:flex-row lg:items-end">
        <div v-for="area in areas" :key="area.title" class="cursor-pointer flex flex-col-reverse justify-between gap-y-2 rounded-2xl bg-white  sm:flex-row-reverse sm:items-end lg:w-full lg:max-w-sm lg:flex-auto lg:flex-col lg:items-start lg:gap-y-5">
          <img :src="area.image" class="" alt="">
        </div>
      </div>
    <!-- Carousel Body -->
